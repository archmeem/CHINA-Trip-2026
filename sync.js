(function(){
  ...
  var PROFILE_MAP_KEY='china-trip-2026-profile-map';
  ...
  function reconcileInitialState(){return stateRef.get().then(function(snap){var localValue=localStorage.getItem(STATE_KEY)||'{}';if(!snap.exists||!snap.data().value){if(localValue!=='{}')return writeCloudState(localValue).then(function(){return false;});return false;}var cloudValue=snap.data().value,mergedValue=mergeTripState(localValue,cloudValue),tasks=[];if(mergedValue!==cloudValue)tasks.push(writeCloudState(mergedValue));return Promise.all(tasks).then(function(){return applyCloudWithoutReload(mergedValue);});});}
  function startRealtimeStateListener(){if(stopStateListener)stopStateListener();stopStateListener=stateRef.onSnapshot(function(snap){if(!snap.exists||!snap.data().value)return;var cloudValue=snap.data().value,localValue=localStorage.getItem(STATE_KEY)||'{}',mergedValue=mergeTripState(localValue,cloudValue);if(mergedValue!==cloudValue&&auth.currentUser)writeCloudState(mergedValue).catch(function(){});applyCloudWithoutReload(mergedValue);updateConnectivityBadge();},function(){addSyncBadge('Offline · saved on device','offline');});}
  function bootFirebase(){if(firebaseStarted||typeof firebase==='undefined')return;firebaseStarted=true;try{firebase.initializeApp(firebaseConfig);auth=firebase.auth();db=firebase.firestore();provider=new firebase.auth.GoogleAuthProvider();stateRef=db.collection('shared').doc('trip-state');auth.onAuthStateChanged(function(user){if(!user){if(stopStateListener){stopStateListener();stopStateListener=null;}var existing=document.getElementById('travelerProfile');if(existing)existing.remove();showLoginButton();updateConnectivityBadge();return;}renderTravelerProfile(user);var lb=document.getElementById('firebaseLogin');if(lb)lb.remove();reconcileInitialState().then(function(){updateConnectivityBadge();startRealtimeStateListener();renderTravelerProfile(user);}).catch(function(){addSyncBadge('Offline · saved on device','offline');renderTravelerProfile(user);});});}catch(e){firebaseStarted=false;addSyncBadge('Offline · saved on device','offline');}}
  function ensureFirebaseSync(){if(firebaseStarted||loadingFirebase||!navigator.onLine)return;if(typeof firebase!=='undefined'){bootFirebase();return;}loadingFirebase=true;var p=Promise.resolve();FIREBASE_SCRIPTS.forEach(function(src){p=p.then(function(){return loadScript(src);});});p.then(function(){loadingFirebase=false;bootFirebase();}).catch(function(){loadingFirebase=false;addSyncBadge('Offline · saved on device','offline');});}
  window.addEventListener('online',function(){updateConnectivityBadge();ensureFirebaseSync();});window.addEventListener('offline',updateConnectivityBadge);updateConnectivityBadge();ensureFirebaseSync();
}());